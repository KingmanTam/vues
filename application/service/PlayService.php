...
class PlayService
{
    ...
    public function setData(): void
    {
        ...
        $params = array();
        if ($row['type_id']==1){
            $params['title'] = '《'.$row['vod_name'].'》第'.App::$args['nid'].'集在线播放'.'-'.App::$domain['name'];
        }else if ($row['type_id']==6){
            $params['title'] = $row['vod_name'].'-'.App::$domain['name'];
        }else{
            $params['title'] = '《'.$row['vod_name'].'》'.$row['vod_remarks'].'在线播放'.'-'.App::$domain['name'];
        }
        $params['keywords'] = $row['vod_name'].'正在播放';
        $params['des'] = $row['vod_name'].'由'.App::$domain['name'].'提供你喜欢的'.$row['vod_name'].'在线观看和免费下载';
        ...
        $content = $row['vod_content'].'<p>《'.$row['vod_name'].'》'.$row['vod_year'].'年上映以来口碑评分都很高,'.$row['vod_blurb'].'</p>';
        AppService::assign('vod_content',$content);
