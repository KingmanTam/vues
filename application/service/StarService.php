...
class StarService
{
    ...
    public function setIndexData(): void
    {
        ...
        foreach ($rows as $row){
            if ($count==1){
                ...
                AppService::assign('actor_hits',$row['actor_hits']);
        ...
        if (App::$args['method']=='index'){
            ...
            $params['title'] = $rows[0]['actor_name'].'最新电影_'.$rows[0]['actor_name'].'最新电视剧_'.$rows[0]['actor_name'].'最新综艺节目'.'-明星-'.App::$domain['name'];
        ...
        if (App::$args['method']=='detail'){
            ...
            AppService::setSEO($params);
            $this->setDetail($rows[0]);
        }
        if (App::$args['method']=='works'){
            $params['title'] = $rows[0]['actor_name'].'参演的电影_'.$rows[0]['actor_name'].'参演的电视剧_'.$rows[0]['actor_name'].'参演的综艺节目'.'-明星-'.App::$domain['name'];
            $params['keywords'] = $rows[0]['actor_name'].'参演的电影,'.$rows[0]['actor_name'].'参演的电视剧,'.$rows[0]['actor_name'].'参演的综艺节目';
            $params['des'] = App::$domain['name'].'提供你喜欢的'.$rows[0]['actor_name'].'的全部资料,'.$rows[0]['actor_name'].$rows[0]['actor_blurb'];
            AppService::setSEO($params);
    ...
    public function setList($name): void
    {
        ...
        foreach ($rows as $row) {
            ...
            $item.= '<li class="col-md-2 col-sm-3 col-xs-4">
              <a class="video-pic loading" data-original="'.$row['vod_pic'].'" href="'.AppService::getLink(7,$par).'" title="'.$row['vod_name'].'">
    ...
    {
        $client = new SolrClient(App::$solr['vod']);
        $client->addField('vod_actor',$name,false);
        $client->setRows(24);
        ...
        $page_url = '/starworks/'.App::$args['id'].'/{page}.html';
    ...
    public function setDetail($bean): void
    {
        $rows = Db::selectAll('select * from mac_vod where vod_id<' . $bean['actor_score_num'] . ' order by vod_id desc limit 4');
        $item_html = '';
        foreach ($rows as $row) {
            $item_html.='<div class="col-md-6 col-sm-6 col-xs-12">
          <div class="col-md-12 col-sm-12 col-xs-12 news-box-txt p-0">
            <div class="col-md-2 col-sm-3 col-xs-3 news-box-txt-l">
              <a class="video-pic loading" href="/role/18201.html" title="'.$bean['actor_name'].$row['vod_name'].'" data-original="'.$row['vod_pic'].'" ></a>
            </div>
            <div class="col-md-10 col-sm-9 col-xs-9 pb-0">
              <div class="news-info">
                <div class="actor-role-top">
                  <strong><a href="/details/'.AppService::dechexId($row['vod_id']).'.html" target="_blank" title="'.$row['vod_name'].'">'.$row['vod_name'].'</a></strong>
                  <div class="more pull-right">
                    <a href="/details/'.AppService::dechexId($row['vod_id']).'.html" class="text-muted hidden-xs" title="详细">详细 <i class="icon iconfont"></i></a>
                  </div>
                </div>
                <div class="actor-role-info" style="min-height:60px;">'.substr($row['vod_content'], 0,150).'…</div>
              </div>
            </div>
          </div>
        </div>';
        ...
        AppService::assign('actor_role',$item_html);
        AppService::assign('star_content',$bean['actor_content']);
    }
}