...
class AppService
{
    ...
    public static function setCommon(): void
    {
        ...
        self::assign('site_url',app::$full_url);
    ...
    {
        $key = App::$domain['cacheId'] . ':page_footer_'.App::$full_url;
        $footer = Cache::get($key);
        if (empty($footer)) {
            $footer = file_get_contents('template/film/html/public/footer.html');
            $footer = str_replace('{mv:site_url}', App::$full_url, $footer);
    ...
    public static function setDPTop(): void
    {
        ...
        if (!(empty($d_top) || empty($top_first))) {
            ...
            Cache::set($key, $html, App::$redis['timeout_;']);
            ...
            foreach ($storys as $sy){
                if ($story_count < 4) {
                    $story_html .= '<li class="list p-0">
                            <a class="pull-left" href="/plot/'.AppService::dechexId($sy['vod_id']).'/'.$sy['story_id'].'.html" title="' . $sy['name'] . '">
    ...
    public static function dechexId($id): string
    {
        $inc_id = App::$domain['inc']+$id;
        return dechex($inc_id);
    }
    public static function hexdecId($id): string
    {

        return hexdec($id)-App::$domain['inc'];
    }
    ...
    public static function getRecommendItem($id){
        $item_id = $id-App::$domain['offset'];
        $key_arr = array();
        for ($i = $item_id-12; $i < $item_id; $i++) {
            array_push($key_arr,'vod:vod_'.$i);
        }
        $result = array();
        $mget_result = Cache::mget($key_arr);
        foreach ($mget_result as $one){
            if ($one){
                array_push($result,json_decode($one,true));
            }
        }
        return $result;
    }
    ...
    public static function getLink($type, $p): string
    {
        ...
        if ($type == 2) {
            return '/film/' . $p . '____.html';
        }
        if ($type == 3) {
            return '/' . App::$type[$p['type_id']] . '/' . $p['class'] . '___1.html';
        }
        if ($type == 4) {
            return '/details/' . AppService::dechexId($p['vod_id']) . '.html';
        }
        if ($type == 5) {
            return '/relay/' . AppService::dechexId($p['vod_id']) . '-' . $p['sid'] . '-' . $p['nid'] . '.html';
        }
        if ($type == 6) {
            return '/star/' . $p . '.html';
        }
        ...
        if ($type == 7) {
            return '/details/' . AppService::dechexId($p['vod_id']) . '/';
        }
        return '/' . $p . '/';
    }
}