...
class StoryService
{
    ...
    public function setLastUpdate(): void
    {
        ...
        foreach ($rows as $row){
            $item_html.='<li class="list1 text-overflow"><em class="key">'.$count.'.</em><a href="/plot/'.$row['story_id'].'.html"
    ...
    public function setIndexDetail(): void
    {
        ...
        AppService::assign('vod_id',AppService::dechexId(App::$args['vod_id']));
        ...
        if (App::$args['story_method']=='all'){
            $pre_content = '<p>'.$row['vod_class'].App::$type_name[$row['type_id']].'《'.$row['vod_name'].'》'.$row['vod_year'].'在'.$row['vod_area'].'上映,'.$row['vod_director'].'执导,'.mb_substr($row['vod_actor'],0,30).'等主演,影片评分'.$row['vod_score'].'分,'.
            App::$domain['name'].'('.App::$full_url.')为你提供《'.$row['vod_name'].'》高清/超清/蓝光/未删减完整版/免费在线观看</p>';
            $content = $pre_content . $row['vod_content'];
            AppService::assign('story_content',$content);
        }
    ...
    public function setIndexEpis($rows, $vod): void
    {
        ...
        foreach ($rows as $row){
            $last_num = $row['epis'];
            $story_name = $row['name'];
            $item_html.='<li><a href="/plot/'.$row['story_id'].'.html">第'.$row['epis'].'集</a></li>';
        }
        if (empty($story_name)){
    ...
    public function setDetailEpis($rows): void
    {
        ...
        foreach ($rows as $row){
            ...
            if ($row['story_id'] == App::$args['story_id']){
                ...
                $epis = $row['epis'];
                $item_html.='<li><a class="active" href="/plot/'.$row['story_id'].'.html">第'.$row['epis'].'集</a></li>';
            }else{
                $item_html.='<li><a href="/plot/'.$row['story_id'].'.html">第'.$row['epis'].'集</a></li>';
            }
        }
    ...
    {
        $rows = AppService::getRecommendItem(App::$args['vod_id']);
        $html = '';
        foreach ($rows as $row){
            $p=[
                'type_id'=>$row['type_id'],
